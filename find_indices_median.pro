...
pro find_indices_median, root=root, $
                       nchain=nchain, $
                       fst_ch=fst_ch, $
                       nforeg=nforeg, $
                       tag=tag, $
...
;##           peak_ind[ipix,iind] = a[im]
           median_ind[ipix,iind] = median( ind_arr[ipix,iind, *] )
;stop
       endfor
   endfor

   write_fits_map,tag+'_median_emissivity.fits', median_ind[*,0],/ring
   write_fits_map,tag+'_median_t.fits', median_ind[*,1],/ring
   write_fits_map,tag+'_median_beta.fits', median_ind[*,2],/ring

stop
   for iind=0,nind[0]-1 do begin
       mollview, ind[*,iind], tit='Mean'
       mollview, median_ind[*,iind], tit='Median'
   endfor

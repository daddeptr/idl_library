pro compare_xfaster_spectra, file1, file2, init=init, ncl=ncl, win=win, otit=otit, res=res, lmax=lmax, lmin=lmin, leg_tags=leg_tags, uleg_pos=uleg_pos, bleg_pos=bleg_pos, reso=reso, xlog=xlog, ylog=ylog, tclfile=tclfile, psout=psout, dops=dops, percent=percent, binning=binning
    ...
    if not keyword_set(reso) then reso = 100
...
;##    cl2 = extract_xfaster_newdata_output(file2,ncl=ncl, lcen=lcen2, cler=cler2)
    ...
    if not keyword_set(dops) then begin
       ...
       plot, l[il], tcl[il,ncl-1]*ll[il], chars=1.5, xtit='!6l', ytit='!6D!dl!n [!7l!6K!u2!n]', xr=[lmin,lmax], tit=otit, xs=1, xlog=xlog, ylog=ylog
    endif else begin
       set_plot,'ps'
       device, file=psout+'_ncl'+strtrim(string(ncl),2)+'.eps', /col, bits=8, /landscape
       plot, l[il], tcl[il,ncl-1]*ll[il], xtit='!6l', ytit='!6D!dl!n [!7l!6K!u2!n]', xr=[lmin,lmax], tit=otit, xs=1, xlog=xlog, ylog=ylog
    endelse
    oplot, l[il], tcl[il,ncl-1]*ll[il], col=245
    oplot, lcen1, btcl, col=245, psym=5
    oplot, lcen1, cl1, psym=4
    errplot, lcen1, cl1-cler1, cl1+cler1
    oplot, lcen2, cl2, psym=6, col=70
    ...
    if not keyword_set(percent) then begin
       ...
       oplot, lcen1, (cler1-cler2), psym=6, col=70

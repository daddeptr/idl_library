...
;;    stop
   ...
   if not (keyword_set(maskfile)) then maskfile='/global/scratch/sd/dpietrob/dx9/maps/ns0128/fsky.fits'
...
;##    if not (keyword_set(nsample))  then nsample = -1l
   ...
   if not (keyword_set(lmax)) then lmax = 250
   ...
   if not (keyword_set(mycommander)) then mycommander = 't'
   if not (keyword_set(thinning)) then thinning = 1
   if not (keyword_set(do_png)) then do_png = 0b
...
; ------ Printing parameters                                                                                                                         
   ...
   printf, 1, "cls_stats, maskfile="+ strtrim(maskfile,2)+ $
     ...
     ", mycommander="+strtrim(mycommander)               + $
...
;;    freq        = ['30', '44', '70', '100', '143', '217', '353', '33', '23', '41', '61', '94']
   ...
   fst_l = 0l
   if (mycommander ne 't') then fst_l = 2l
   lst_l = lmax
   ncl = lmax+1
   

   cl=fltarr(nsample,nspec,2*ncl)

   l = findgen(ncl)
   if (mycommander ne 't') then l = l+2
...
;         print, tot_isamp
         ...
         if ( (ts*100) eq tot_isamp ) then begin
             print, tot_isamp
             if not (do_residuals) then save, filename=tag+'.res.sav', foreg, cmb, cmb2, cl, chi2, nsample, nsampch, nside, tot_isamp, amp, amp2, ind, ind2, namp, nind, glb_chi2, ncl, nl, fst_samp, nchain, fst_ch
             if (do_residuals) then save, filename=tag+'.res.sav', foreg, cmb, cmb2, cl, chi2, residuals, nsample, nsampch, nside, tot_isamp, amp, amp2, ind, ind2, namp, nind, glb_chi2, ncl, nl,fst_samp,nchain,fst_ch
         endif
         ...
         if (cls_samp eq 'on') then begin
            if not (do_pol) then begin
               readcol,root+'cl_'+sch+'_'+ssam+'.dat', l, tt, /silent
               half_set = n_elements(l) / 2
               lmx = max(l, imx)
               cl_tmp[1, l[0:half_set-1] ] = tt[0:half_set-1]
               cl_tmp[1, lmx+1+l[half_set:*] ] = tt[half_set:*]
;print, lmx, imx
;print, l[0:half_set-1]
;print, lmx+1+l[half_set:*]
;stop
            endif
            if (do_pol) then begin
               readcol,root+'cl_'+sch+'_'+ssam+'.dat', l, tt, te, tb, ee, eb, bb
               if (do_warn) then begin
                   do_warn = False
...
;             close, 1
         ...
         chi2[tot_isamp] = total(chisq[gpix]) / ngpix / nband
...
;;         freq = ['030', '044', '070', '100', '143', '217', '353']
   ...
   window, 20, xs=720*0.8, ys=450*0.8 & plot, chi2, yr=[0.9*min(chi2),1.1*max(chi2)], ys=1, ytit='!7v!u2!n', xtit='!17Sample'
   oplot, chi2, psym=1, col=245
   if (do_png) then write_png, tag+'_chi2.png', tvrd(/true)
   chihst =  histogram(chi2, nbins=50, locations=achi);, min=mean(chi2)-4.*stddev(chi2), max=mean(chi2)+4.*stddev(chi2) )
   window, 21, xs=720*0.8, ys=450*0.8 & plot, achi, chihst/total(chihst), ys=2, xtit='!7v!u2!n',ytit='!17P', psym=10
   if (do_png) then write_png, tag+'_chi2_hist.png', tvrd(/true)
...
;stop
   ...
   if (cls_samp eq 'off') then begin
      ...
      if (1b) then begin
         ...
         ccc = cmb
         remove_dipole, ccc, mask[*,0], ordering='ring', nside=ns, /noremove, dipole=cmbdip, monopole=cmbmon
         print, cmbdip
         write_fits_map, tag+'_recCMB_ns'+s_ns+'_RING.fits',ccc, /ring,units='!7l!17K'
         write_fits_map, tag+'_rmsCMB_ns'+s_ns+'_RING.fits',cmbv, /ring,units='!7l!17K'
         ccc = cmb
;##         if (bpix[0] ne -1) then ccc[bpix] = bad_value
         my_ls_mollview, ccc, tit='!17Reconstructed CMB: '+tag, grat=[10,10], px=800, win=31
;         mollview, ccc, min=-extr, max=extr, chars=1.5, tit='!17Reconstructed CMB', units='!7l!8K CMB', ps=tag+'_cmb.eps'
         if do_png then my_ls_mollview, ccc, tit='!17Reconstructed CMB', png=tag+'_cmb.png', window=-1, no_monopole=1, gal_cut=30

         ccc = cmbv
         if (bpix[0] ne -1) then ccc[bpix] = bad_value
         my_ls_mollview, ccc, tit='!17Reconstructed CMB sigma: '+tag, win=30, px=600
      endif else begin
   ...
   if (cls_samp eq 'on') then begin
       ...
       ccc = smcmb
       if (bpix[0] ne -1) then ccc[bpix] = bad_value
       mollview, ccc, min=-extr, max=extr, chars=1.5, tit='!17Reconstructed CMB: '+tag, px=800
       ccc = cmb
       if (bpix[0] ne -1) then ccc[bpix] = bad_value
       mollview, ccc, min=-extr, max=extr, chars=1.5, tit='!17Reconstructed CMB: '+tag, px=800
       ...
       ccc = smcmbv
       if (bpix[0] ne -1) then ccc[bpix] = bad_value
       mollview, ccc, min=min(smcmbv[where(mask gt 0.)]*mask[where(mask gt 0.)]), chars=1.5, tit='!17Reconstructed CMB sigma: '+tag,px=800
...
endelse
   ...
   mollview, cc, chars=1.5, tit='!7v!u2!17', max=3*nband, grat=[30,30], px=600, win=0
;   mollview, cc, chars=1.5, tit='!7v!u2!17', max=4*nband, ps=tag+'_glb_chi2.eps'
   if do_png then mollview, cc, chars=1.5, tit='!7v!u2!17', max=3*nband, png=tag+'_glb_chi2.png', window=-1
   ...
   if (fg_pix) then begin
       ...
       for iamp=1,namp[0] do begin 
           if (iamp-1 ne icmb) then begin
               ...
               cc[bpix] = -1.6375e30
               print, ''
               print, ' --> Amplitude ', iamp
               remove_dipole, cc, mask, nside=ns, ordering='ring', /noremove, monopole=ampmon, dipole=ampdip;, /onlymonopole;, gal_cut=80
               print, ampdip

;##               mollview, cc, chars=1.5, tit='!17Amp '+string(iamp, format='(i2.2)'), grat=[10,10], px=600, win=iamp+2 $
;##                 , units='!7l!8K Antenna', /hist ;max=250;, no_monopole=1, gal_cut=80
               my_ls_mollview, cc, tit='!17Amp '+string(iamp, format='(i2.2)'), grat=[10,10], px=600, win=iamp+2
               cc = amp[*,iamp-1]

;##               if do_png then mollview, amp[*,iamp-1], chars=1.5, tit='!17Amp '+string(iamp, format='(i2.2)'),  /hist, grat=[10,10], units='!7l!8K Antenna', window=-1, png=tag+'_amp_'+string(iamp,format='(i2.2)')+'.png', min=0
               if do_png then my_ls_mollview, amp[*,iamp-1], tit='!17Amp '+string(iamp, format='(i2.2)'),  grat=[10,10], window=-1, png=tag+'_amp_'+string(iamp,format='(i2.2)')+'.png'
           endif
       endfor
       print, '- :DP Indices next...'
       stop
       for iind=1,nind[0] do begin
           cc = ind[*,iind-1]
           if (bpix[0] ne -1) then cc[bpix] = -1.6375e30
           mn = mean(cc[gpix])-2.*stddev(cc[gpix])
           mx = mean(cc[gpix])+2.*stddev(cc[gpix])

           mollview, cc, chars=1.5, tit='!17Ind '+string(iind, format='(i2.2)'), grat=[10,10], px=600, win=iind+iamp+2, min=mn, max=mx
           if do_png then mollview, ind[*,iind-1], chars=1.5, tit='!17Ind '+string(iind, format='(i2.2)'), grat=[10,10], window=-1, png=tag+'_ind_'+string(iind,format='(i2.2)')+'.png', min=mn, max=mx
       endfor
...
;;                write_png,'hke_'+tag+'_ClsHist.png',tvrd(/true)
               if do_png then write_png,tag+'_ClsHist.png',tvrd(/true)
               ...
               if (do_corr) then begin
                   ...
                   if do_png then write_png,tag+'_cls_correlation.png',tvrd(/true)
               end
...
;       errplot, l[2:*], (err[ispec,2:*,0]), (err[ispec,2:*,1]), col=70
       ...
       if not (do_pol) then begin
           if do_png then write_png,tag+'_Cls.png',tvrd(/true)
       if not (do_wmap) then begin
           window, 29
           ...
           if do_png then write_png,tag+'_Cls_diff.png', tvrd(/true)
       endif
       if (do_wmap) then begin
           if (1b) then begin
...
;               oplot, wmapl, wmapl*0.+0.
               ...
               if (0b) then begin
                   ...
                   if do_png then write_png,tag+'_ilc-cmb_cls_comp.png',tvrd(/true)
               endif
           endif
       endif
...
;;    oplot, l, ncls/gb^2*ll, line=3
   ...
   oplot, l[2:*], ctp3cls[2:*]*ll[2:*]-cls[2:*]*ll[2:*]/gb[2:*]^2/fsky, col=205
    if do_png then write_png,tag+'_convCls_2.png',tvrd(/true)

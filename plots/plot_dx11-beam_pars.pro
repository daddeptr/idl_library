...
;stop
   ...
   run_tags[1] = 'DX11!dpre!n 143u-IN DX11!dpre!n beam'
...
;##   colors = findgen( nfold) / (nfold-1) * 245
   colors = [245, 0, 70, 210, 40, 245, 150]
...
;##   !p.multi = [0,3,ceil(npars/3)]
   ...
   if dops then begin
       set_plot, 'ps'
;##       device, file='dx11_runs-beam.eps', /col, bits=8, /landscape
       device, file='beamstudy_dx11_dx11beam_pars.eps', /col, bits=8, /landscape
   endif else begin
   ...
   for ip=0,npars-1 do begin
       ...
       if dops then plot, /nodata, [p_range[ip,0], p_range[ip,1]], [0,1.1], xtit=p_lab[ip], ytit='P', $
...
;       oplot ,[0,0.1],[0.75-0.15*ixxx,0.75-0.15*ixxx], col=colors[ixxx], thick=2
       xyouts, 0., 0.75-0.175*ixxx, run_tags[ixxx], col=colors[ixxx];, chars=1.25
   endfor
;   stop

...
;
   ...
   chains[1] = 'base_planck_tauprior'
   chains[2] = 'xf_chains_T_graca'
   chains[3] = 'xf_chains_T_common_ctp'
   chains[4] = 'xf_chains_T_Tmask_ctp'
   chains[5] = 'xf_chains_T_Tmask_const2'
   chains[6] = 'compsep'
   chains[7] = 'xf_chains_T_const2'
   chains[8] = 'xf_chains_TP_const2'
   ...
   symbs = [4,4,5,5,5,5,5,5,5,6,5]
   run_tags = [folders,'','']
   run_tags[*] = ''
   run_tags[0] = 'CamSpec (T)'
   run_tags[1] = 'CamSpec (T,tau-prior)'
   run_tags[2] = 'ddx9-Smica (T) apo30,ctp'
   run_tags[3] = 'ddx9-Smica (T) common,ctp'
   run_tags[4] = 'ddx9-Smica (T) Tmask,ctp'
   run_tags[5] = 'ddx9-Smica (T) Tmask,const'
   run_tags[6] = 'ddx9-Smica Paper'
   run_tags[7] = 'predx11-Smica (T) Tmask,const'
   run_tags[8] = 'predx11-Smica (TP) Tmask,const'

   colors = [245, 235, 70, 90, 105, 120, 210, 200, 45 ]
...
;##   xyouts, 0.5, 0.65, 'DX11'
   ...
   for ixxx=4,6 do begin
       xyouts, 0.25, 0.65-0.25*(ixxx-3), run_tags[ixxx], col=colors[ixxx]
   endfor
   for ixxx=7,nfold-1 do begin
       xyouts, 0.5, 0.65-0.25*(ixxx-6), run_tags[ixxx], col=colors[ixxx]
   endfor

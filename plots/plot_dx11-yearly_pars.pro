;## /global/scratch2/sd/hou/us_planck/for_davide/xfaster_cosmomc/runs/run_plg131024.readme
   ...
   if init then begin
       mollview, findgen(12l*16^2), win=3, px=550
       loadct, 39
       !p.color=0
       !p.background=255
   endif
...
;
   ...
   freq = ['100','143','217']
   ggrun = [ [grun100],[grun143],[grun217] ]
;   print, grun
;;stop   

   for irun=1,1 do begin
...
;stop
   ...
   run_tags[1] = 'yr '+freq[irun]+': clean-IN'
   run_tags[2] = 'yr '+freq[irun]+': raw'
   run_tags[3] = 'yr '+freq[irun]+': clean-OUT'

   run_tags[1] = 'yr 100GHz: clean-IN'
   run_tags[2] = 'yr 143GHz: clean-IN'
   run_tags[3] = 'yr 217GHz: clean-IN'

;##   for i=1,n_elements(grun)-1 do begin
;##;       tag=strsplit(folders[i],'/',/extract)
;##       run_tags[i] = files[grun[i]-1]
;##   endfor
;##   colors = findgen( nfold) / (nfold-1) * 245
;##   colors = [235, 70, 70, 70, 100, 100, 100, 210, 210, 210] ;150, 150,180, 180, 0, 0, 120, 120]
;   colors = [235, 20, 70, 90, 20, 20, 70, 70, 70, 90, 90, 90];, 70, 100, 100, 100, 210, 210, 210] ;150, 150,180, 180, 0, 0, 120, 120]
   colors = [235, 0, 70, 210, 20, 20, 70, 70, 70, 90, 90, 90];, 70, 100, 100, 100, 210, 210, 210] ;150, 150,180, 180, 0, 0, 120, 120]
...
;##   !p.multi = [0,3,ceil(npars/3)]
   ...
   if dops then begin
       set_plot, 'ps'
;       device, file='dx11_runs-yearly_'+freq[irun]+'.eps', /col, bits=8, /landscape
       device, file='dx11_runs-yearly_allFreq_IN_T.eps', /col, bits=8, /landscape
   endif else begin
...
;##       xtic = findgen(nxtic+1)*(max(likes[*,*,ip,*,ival])-min(likes[*,*,ip,*,ival]))/nxtic + min(likes[*,*,ip,*,ival])
       xtic = findgen(nxtic+1)*( p_range[ip,1]-p_range[ip,0])/nxtic + p_range[ip,0]

       nytic = 4
;##       ytic = findgen(nytic+1)*(max(likes[*,*,ip,*,ih])-min(likes[*,*,ip,*,ih])) / nytic + min(likes[*,*,ip,*,ih])
       ytic = findgen(nytic+1) / nytic
       ...
       if not dops then plot, /nodata, [p_range[ip,0], p_range[ip,1]], [0,1.1], xtit=p_lab[ip], ytit='P', $
         xtickv=xtic, xticks=nxtic, yticks=nytic, ytickv=ytic, ytickname=yticnames,chars=2.5, xs=1, ys=1

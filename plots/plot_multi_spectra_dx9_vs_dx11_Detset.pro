...
;
;       'dx9_ds_1-2_Imap_143_nominal_extMask_545_undusted_insideM_split_flat_ncl4_v1.8_ns2048_uK_hrhs_xfcl_l3000_Tmask_X_Pmask_l4000.newdat', $
...
;                                 '217 '+['raw','clean-OUT','clean-IN'], $
                                 'dx11_pre Full dx11-beam ', $
                                 'dx11_pre Full dx9-beam ', $
                                 'dx9 Nominal dx9-beam ', $
                                 'dx9 Full dx9-beam ', $
                                 'dx9 Full ffp7-beam ' $
...
;##           if not xlog then device, file='newDetset_spectra.eps', /col, bits=8, /landscape else device, file='newDetset_spectra_xlog.eps', /col, bits=8, /landscape
           if not xlog then device, file='beamStudy_ddx9_vs_predx11_spectra.eps', /col, bits=8, /landscape else device, file='newDetset_spectra_xlog.eps', /col, bits=8, /landscape
       endif else begin
...
;##       !p.charsize=1.25
       ...
       oplot, [0.35,0.35],[0.4,0.4], psym=psyms[2], col=colors[2]
       xyouts, 0.357, 0.35, run_tags[2], col=colors[2], chars=1.

       oplot, [0.35,0.35],[0.1,0.1], psym=psyms[3], col=colors[3]
       xyouts, 0.357, 0.05, run_tags[3], col=colors[3], chars=1.

       oplot, [0.65,0.65],[0.4,0.4], psym=psyms[4], col=colors[4]
       xyouts, 0.657, 0.35, run_tags[4], col=colors[4], chars=1.

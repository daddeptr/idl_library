...
chname = False
correlate_maps = False

dataset = sets[0]

if (chname) then begin
    sversion='v1.0.128'
    spawn, 'mkdir -p /global/scratch/sd/dpietrob/commander-ruler/'+dataset+'/'+sversion
    comp = ['cmb','fg_co_amp','fg_dust_amp','fg_lowfreq_amp']
    for ic=0,3 do begin
        spawn, 'ln -s /global/scratch/sd/dpietrob/dx9/output/glss/pix_7b/dx9_7b_mondip_avrg_'+incomp[ic]+'.fits '+ $
        '/global/scratch/sd/dpietrob/commander-ruler/'+dataset+'/'+sversion+'/'+dataset+'_commander-ruler_'+sversion+'_'+comp[ic]+'_uK.fits'
        spawn, 'ln -s /global/scratch/sd/dpietrob/dx9/output/glss/pix_7b/dx9_7b_mondip_hr1_avrg_'+incomp[ic]+'.fits '+ $
        '/global/scratch/sd/dpietrob/commander-ruler/'+dataset+'/'+sversion+'/'+dataset+'_commander-ruler_'+sversion+'_'+comp[ic]+'_hr1_uK.fits'
        spawn, 'ln -s /global/scratch/sd/dpietrob/dx9/output/glss/pix_7b/dx9_7b_mondip_hr2_avrg_'+incomp[ic]+'.fits '+ $
        '/global/scratch/sd/dpietrob/commander-ruler/'+dataset+'/'+sversion+'/'+dataset+'_commander-ruler_'+sversion+'_'+comp[ic]+'_hr2_uK.fits'

        read_fits_map, '/global/scratch/sd/dpietrob/dx9/output/glss/pix_7b/dx9_7b_mondip_hr1_avrg_'+incomp[ic]+'.fits', map1
        read_fits_map, '/global/scratch/sd/dpietrob/dx9/output/glss/pix_7b/dx9_7b_mondip_hr2_avrg_'+incomp[ic]+'.fits', map2

        diff = (map1-map2) /2.

        file = '/global/scratch/sd/dpietrob/commander-ruler/'+dataset+'/'+sversion+'/'+dataset+'_commander-ruler_'+sversion+'_'+comp[ic]+'_error_uK.fits'
        write_fits_map, file, diff, /ring, units='!7l!6K'
        mollview, file
    endfor
stop
endif
...
;##    iris_file = 'IRIS_nohole_4_2048.fits'
    ...
    read_fits_map,'/global/scratch/sd/dpietrob/commander-ruler/'+dataset+'/'+sversion+'/'+dataset+'_commander-ruler_'+sversion+'_cmb_uK.fits', dx8, order=dpord, nside=dpns 
    if ( (dpord eq 'ring') or (dpord eq 'RING') ) then begin
        print, ' - Reordering: ', dpord, ' --> NEST'
        dx8 = reorder(dx8, in='ring', out='nest')
    endif

    read_fits_map,'/global/scratch/sd/dpietrob/commander-ruler/'+dataset+'/'+sversion+'/'+dataset+'_commander-ruler_'+sversion+'_fg_co_amp_uK.fits', co, order=dpord, nside=dpns 
    if ( (dpord eq 'ring') or (dpord eq 'RING') ) then begin
        print, ' - Reordering: ', dpord, ' --> NEST'
        co = reorder(co, in='ring', out='nest')
    endif

    read_fits_map,'/global/scratch/sd/dpietrob/commander-ruler/'+dataset+'/'+sversion+'/'+dataset+'_commander-ruler_'+sversion+'_fg_dust_amp_uK.fits', dust, order=dpord, nside=dpns 
    if ( (dpord eq 'ring') or (dpord eq 'RING') ) then begin
        print, ' - Reordering: ', dpord, ' --> NEST'
        dust = reorder(dust, in='ring', out='nest')
    endif
    ...
    if (mapns ne dpns) then begin
        help, dpns, mapns

        Nside = min( [dpns, mapns] )
        print, mapns,' vs ',dpns,' --> downgrading to ',Nside

        ud_grade, dx8, dx8, nside_out=nside, order_in='nest'
        ud_grade, co, co, nside_out=nside, order_in='nest'
        ud_grade, dust, dust, nside_out=nside, order_in='nest'
        ud_grade, template, template, nside_out=nside, order_in='nest'

        mollview, dx8, min=-500, max=500, chars=1.5, win=1, /nest
        mollview, co, min=-500, max=500, chars=1.5, win=2, /nest
        mollview, dust, min=-500, max=500, chars=1.5, win=3, /nest
        mollview, template, min=-500, max=500, chars=1.5, win=4, /nest
    endif

    
    gmask = fltarr(Npix)+1.
    mpix = where( (dx8 eq 0.) or (dx8 eq -1.6375e30) or (finite(dx8, /nan) eq True) or (finite(template, /nan) eq True) or (template lt 0.) )
    help, mpix
    ...
    for i=0, low_Npix-1 do begin
        ...
        if (corr_gp[0] ne -1) then begin
            correlation_maps[i,0] = correlate(dx8[pix_list[corr_gp]], template[pix_list[corr_gp]])
            correlation_maps[i,1] = correlate(co[pix_list[corr_gp]], template[pix_list[corr_gp]])
            correlation_maps[i,2] = correlate(dust[pix_list[corr_gp]], template[pix_list[corr_gp]])
        endif
    endfor

    mollview, correlation_maps[*,0], chars=1.5, min=-1, max=1, tit='!6CR cmb - HI Correlation', /nest, win=11
    mollview, correlation_maps[*,1], chars=1.5, min=-1, max=1, tit='!6CR co - HI Correlation', /nest, win=12
    mollview, correlation_maps[*,2], chars=1.5, min=-1, max=1, tit='!6CR dust - HI Correlation', /nest, win=13
...
;stop
    correlation_maps = reorder(correlation_maps,in='NESTED', out='RING')

    mollview, correlation_maps[*,0], chars=1.5, min=-1, max=1, tit='!6CR cmb - HI Correlation', win=-1, png='dx9_cmb-HI_correlation.png'
    mollview, correlation_maps[*,1], chars=1.5, min=-1, max=1, tit='!6CR co - HI Correlation', win=-2, png='dx9_co-HI_correlation.png'
    mollview, correlation_maps[*,2], chars=1.5, min=-1, max=1, tit='!6CR dust - HI Correlation', win=-3, png='dx9_dust-HI_correlation.png'

    write_tqu, dataset+'_correlation_maps_lab_ns'+strtrim(string(low_Nside),2)+'.fits', correlation_maps, /ring
